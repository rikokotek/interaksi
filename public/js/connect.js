        ...
        <div class="card">
          ...
          <div class="form-group" style="margin-top:10px;">
            <label style="display:flex;align-items:center;gap:6px;">
              ...
              YouTube Proxy URL (Opsional)
            </label>
            <input type="text" id="yt-proxy-input" class="form-control" placeholder="http://user:pass@ip:port" value="${AppState.ytConnectionState?.proxyUrl || ''}" />
          </div>
...
async function connectYoutube() {
  ...
  const proxyInput = document.getElementById('yt-proxy-input');
  ...
  try {
    await apiFetch('/api/youtube/connect', { method: 'POST', body: { channelId, isLiveId, proxyUrl } });
  } catch (err) {
    showToast('Gagal connect: ' + err.message, 'error');
